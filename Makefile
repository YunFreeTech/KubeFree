...
KUBEFREEDIR=$(BASEPATH)/web/kubefree
...
APP_NAME=kubefree-server

build_web_kubefree:
	cd $(KUBEFREEDIR) && npm install && npm run-script build
build_web_dashboard:
	cd $(DASHBOARDDIR) && npm install && npm run-script build
build_web_terminal:
	cd $(TERMINALDIR) && npm install && npm run-script build

build_web: build_web_kubefree build_web_dashboard build_web_terminal
...
build_docker:
	docker build -t YunFreeTech/kubefree-server:master .
