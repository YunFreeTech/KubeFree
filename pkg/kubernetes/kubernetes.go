...
func (k *Kubernetes) CleanManagedClusterRole() error {
	...
	labels := []string{
		fmt.Sprintf("%s=%s", LabelManageKey, "kubefree"),
...
func (k *Kubernetes) CreateCommonUser(commonName string) ([]byte, error) {
	...
	if minor > 18 {
		csr := certv1.CertificateSigningRequest{
			ObjectMeta: metav1.ObjectMeta{
				Name: fmt.Sprintf("%s-%s-%d", commonName, "kubefree", time.Now().Unix()),
			},
...
func (k *Kubernetes) CreateAppMarketCRD() error {
	...
	_, err = client.ApiextensionsV1().CustomResourceDefinitions().Get(context.TODO(), "appmarkets.kubefree.org", metav1.GetOptions{})
	...
	crd := &apiextensionv1.CustomResourceDefinition{
		ObjectMeta: metav1.ObjectMeta{Name: "appmarkets.kubefree.org"},
